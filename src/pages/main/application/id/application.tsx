...
import {
  ...
  Clock,
  XCircle,
  ArrowLeft, // Добавлена иконка стрелки назад
} from "lucide-react";
...
const otpSchema = z.object({
  code: z
    ...
    .max(6, "Код должен содержать максимум 6 символов"),
});
...
export default function ApplicationId() {
  ...
  const { data: application, isLoading, refetch } = useApplicationDetail(id);
  ...
  const handleFileUpload = async () => {
    ...
    try {
      ...
      setUploadFile(null);
      refetch(); // Обновляем данные после загрузки
    } catch (error) {
  ...
  // Обработчик отправки заявки (OTP процесс)
  const handleSendApplication = async () => {
    ...
    try {
      ...
      // 1. Запрашиваем OTP
      const otpResult = await sendOtp.mutateAsync(application.id);
  ...
  const handleVerifyOtp = async (data: OtpFormValues) => {
    ...
    try {
      ...
      // 3. Отправляем email
      await sendEmail.mutateAsync(application.id);

      setEmailProcessStep("completed");

      // Обновляем данные заявки после успешной отправки
      await refetch();
  ...
  // Определяем, показывать ли ответ кредитора (для статуса "DONE")
  const showCreditorResponse =
    application.status === "DONE" && application.creditor_response?.file;

  // Проверяем наличие документов
  const hasDocument = application.document?.file;
  const hasContract = application.contract?.file;

  // Определяем статус загрузки для кнопки
  const isSending =
    sendOtp.isPending || verifyOtp.isPending || sendEmail.isPending;
    ...
    <>
      <div className="container mx-auto px-4 py-8 max-w-5xl">
        ...
        <div className="mb-6">
          ...
          <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 ml-12 sm:ml-0">
